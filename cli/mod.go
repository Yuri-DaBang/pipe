...
const (
	...
	CONF_FILE  = "./items/libs/conf.json"
	LIBS_DIR   = "./nuepkgs"
)
...
func pushFile(filePath string, password string) error {
	...
	request, err := http.NewRequest("POST", API_URL+"push", body)
	if err != nil {
...
func pullFile(packageName string, authToken string) error {
	request, err := http.NewRequest("GET", API_URL+"pull/"+packageName, nil)
	if err != nil {
		return fmt.Errorf("could not create request: %v", err)
	}
...
func main() {
	if len(os.Args) < 3 {
		fmt.Println("Usage: go run mod.go <push|pull> <file_path> [-password <password> | -auth <auth_token>]")
		return
	}
	...
	default:
		fmt.Println("Unknown command. Use 'push' or 'pull'.")
	}
}
